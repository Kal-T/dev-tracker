...
const stats = computed(() => {
  ...
  const priorityCounts = {
    ...
    [TASK_PRIORITY.MEDIUM]: taskStore.tasks.filter(t => t.priority === TASK_PRIORITY.MEDIUM).length,
...
<template>
  <div class="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
    <div class="flex items-center justify-between">
      ...
      <div class="bg-blue-50 text-blue-700 px-4 py-1 rounded-full text-sm font-bold border border-blue-100">
        Live Metrics
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      <div class="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
        <p class="text-xs font-black text-slate-400 uppercase tracking-widest mb-2">Completion Rate</p>
